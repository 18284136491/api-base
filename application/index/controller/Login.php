...
class Login extends \app\common\controller\Base
{
    ...
    public function signUp(\think\Request $request)
    {
        ...
        if(!$vali_res){
            return \GuzzleHttp\json_encode(['code' => 1, 'msg' => $validate->getError()]);
        }

        // 用户验证
        $member = \think\Db::name('member')->where('phone', $param['phone'])->find();
        if($member){
            return \GuzzleHttp\json_encode(['code' => 2, 'msg' => '手机号已经被占用']);
        }
        ...
        if(!$register){
            return \GuzzleHttp\json_encode(['code' => '3', 'msg' => '操作失败，请稍后再试']);
        }
        return \GuzzleHttp\json_encode(['code' => 200, 'msg' => '操作成功']);
    }
    ...
    {
        $param = $request->param();
        $data = \GuzzleHttp\json_decode(\Cache::get($param['token']), true);
        ...
        $member = \think\Db::name('member')->where('phone', $param['phone'])->find();
        if(!$member){
            return \GuzzleHttp\json_encode(['code' => 2, 'msg' => '用户不存在']);
        }

        // 密码验证
        $checkpwd = checkPwd($param['password'], $member['password']);
        if(!$checkpwd){
            return \GuzzleHttp\json_encode(['code' => 3, 'msg' => '密码错误']);
        }
    ...
    public function sendCode(\think\Request $request)
    {
        ...
        // 发送验证码
        \Cache::set($param['phone'], $code, config('_phoneCode'));
        return \GuzzleHttp\json_encode(['code' => 200, 'msg' => '操作成功']);
//        return \GuzzleHttp\json_encode(['code' => 200, 'msg' => '操作成功', 'data' => ['code' => $code]]);
    }
    ...
    private function securityCode($phone, $code)
    {
        ...
        if(!$cache_code){
            return \GuzzleHttp\json_encode(['code' => 2, 'msg' => '验证码过期或已超时，请从新发送']);
        }
        if($cache_code !== $code){
            return \GuzzleHttp\json_encode(['code' => 3, 'msg' => '验证码错误']);
        }
        \Cache::rm($phone);
    }
